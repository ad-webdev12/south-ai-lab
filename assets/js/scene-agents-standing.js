...
(function () {
  ...
  SAIL.scenes.agents = function (S, root) {
    ...
    var floorY = bar ? bar.offsetTop : S.H - 120, Z = wide ? clamp(window.innerHeight * 0.00084, 0.5, 0.82) : clamp(S.W * 0.0019, 0.55, 0.8), U = 133 * Z;
    var under = root.querySelector(".ghero-copy"), deskY = under ? under.getBoundingClientRect().top - root.getBoundingClientRect().top - (wide ? 8 : 26) : floorY - U * 0.5;
    var homeX = wide ? S.W * 0.68 : S.W * 0.62, G = 2300, L1 = 174 * Z, L2 = Math.hypot(GRIP["1"][0] - 732, GRIP["1"][1] - 386) * Z, timeScale = 1, slowT = 0;
    ...
    if (mast) mast.classList.remove("wrecked"); if (logo) logo.style.transform = ""; if (restoreBtn) restoreBtn.hidden = true; if (panel) panel.hidden = true; root.setAttribute("data-state", "calm");
    ...
    var letters = [].map.call(h1 ? h1.querySelectorAll(".l") : [], function (el, i) {
      ...
      var btn = el.querySelector("button"); if (!btn) { btn = document.createElement("button"); btn.type = "button"; btn.className = "lb"; btn.setAttribute("aria-label", "Knock the letter " + el.textContent + " loose"); btn.setAttribute("data-letter", String(i)); el.appendChild(btn); }
      return { el: el, btn: btn, idx: i, ch: el.textContent.trim().charAt(0), hx: 0, hy: 0, w: 0, h: 0, x: 0, y: 0, rot: 0, sc: 1, vx: 0, vy: 0, vr: 0, gf: 1, state: "home", t: 0, bounced: 0, big: 0 };
    });
    function measure() {
    ...
    function knock(l) { if (!l || l.state !== "home") return false; announce("The letter " + l.ch + " fell. The agent is restoring the title."); if (level < 1) hintText("The agent is restoring the title."); l.state = "loose"; l.t = 0; l.bounced = 0; l.gf = 1; l.vx = rnd(-130, 130) + (wide && l.hx < S.W * 0.2 ? 90 : 0); l.vy = rnd(-120, -40); l.vr = rnd(-5, 5); return true; }
    function lockIn(l, hard) { l.state = "home"; if (!restoring && !seq && letters.every(function (o) { return o === l || o.state === "home"; })) { announce("Title complete."); if (level < 1) hintText("Tap a letter to knock it loose."); } l.x = l.hx; l.y = l.hy; l.rot = 0; l.sc = 1; l.big = 0; l.el.classList.remove("lock"); void l.el.offsetWidth; l.el.classList.add("lock"); fx.push({ kind: "ring", x: l.hx, y: l.hy, r: l.h * (hard ? 0.9 : 0.5), t: 0, life: hard ? 0.5 : 0.35 }); if (hard) shake = Math.max(shake, 0.35); }
    ...
    var job = null, work = { i: 0, t: 0, k: 0 }, knocks = [], level = 0, clock = 0, sinceMeasure = 0, cue = null, cueAt = 5, variant = 0, shake = 0, fx = [], dust = [], heat = 0, heatTo = 0, warm = 0, warmTo = 0, cool = 0, coolTo = 0, spot = 1, spotTo = 1, flicker = 0, lamp = { a: 0, va: 0 };
    var track = null, seq = null, wrecked = false, restoring = null, wreck = null, cracks = [], monitor = ["TASK: restore title", "STATE: monitoring"], card = null, divider = 0, tilt = null;
    ...
    function stepJob(dt) {
      var j = job, l = j.l; track = l.state === "rest" || l.state === "held" ? { x: l.x, y: l.y } : track;
      var quick = 1 + level * 0.45, focus = { x: l.x, y: l.y }; j.t += dt;
    ...
    function startSeq() {
      seq = { t: 0, did: {}, pre: Math.abs(rb.x - homeX) > 14 ? 0.001 : 0 }; job = null; track = null; wreck = buildWreck(); heatTo = 1; spotTo = 1.7; rb.leanTo = 0; variant = (variant + 1 + ((Math.random() * 2) | 0)) % 3;
      ...
      letters.forEach(function (l) { if (l.state === "home" || l.state === "cue") { l.state = "shiver"; l.t = 0; } else if (l.state === "held" || l.state === "magnet") loose(l, 0, 0, 1); });
      seq.R = find("R"); seq.A = find("A"); if (restoreBtn) restoreBtn.hidden = true;
    }
    ...
    function stepSeq(dt) {
      ...
      var t = (seq.t += dt), a0 = rb.arms[0], a1 = rb.arms[1], sy = shoulderY(), slamX = rb.x + U * 0.6, focus = { x: S.W * 0.25, y: 140 }, R = seq.R, A = seq.A, V = variant;
      ...
      // beat 1: it stops and looks up at the title. The light goes red.
      if (t < 1.0) { focus = { x: S.W * 0.25, y: 140 }; }
      if (once("flash", 0.8)) fx.push({ kind: "screen", t: 0, life: 0.22 });
      // beat 2: one hand comes up and slams the desk. The mounted letters jump loose, and two slide down.
      if (t > 0.75 && t < 1.24) { a1.tx = rb.x + U * 0.75; a1.ty = sy - U * 1.05; a1.speed = 12; a1.curlTo = 1; }
      if (t >= 1.24 && t < 1.7) { a1.tx = slamX; a1.ty = deskY - U * 0.03; a1.speed = 30; focus = { x: slamX, y: deskY }; }
      if (once("slam", 1.33)) {
        shake = 1; slowT = 0.6; fx.push({ kind: "shock", x: slamX, y: deskY, t: 0, life: 1.1 }); lamp.va += 5;
        for (var i = 0; i < 18; i++) dust.push({ x: slamX + rnd(-40, 40), y: deskY - 4, vx: rnd(-320, 320), vy: rnd(-300, -40), t: 0, life: rnd(0.8, 2), s: rnd(1, 2.6), g: 300 });
        letters.forEach(function (l, i) { if (l === R || l === A) return; if (l.state !== "shiver" && l.state !== "rest") return; var wgt = 0.75 + l.w / 60, slide = i % 9 === 3; loose(l, slide ? rnd(60, 160) : rnd(-40, 40), slide ? rnd(-60, 20) : -rnd(120, 260) / wgt, wgt); l.vr = slide ? rnd(2, 5) : rnd(-2, 2); });
        tiles.concat(papers).forEach(function (o) { o.free = true; o.vx = (o.x - slamX) * rnd(1, 2) + rnd(-120, 120); o.vy = rnd(-600, -250); o.vr = rnd(-8, 8); });
        fly(R, function () { return { x: rb.x - U * 0.95, y: shoulderY() - U * 1.55 }; }, 0.85, 1.7, 7, U * 2.2);
        if (V === 1) { A.state = "shiver"; } else fly(A, function () { return { x: A.hx + 40, y: -260 }; }, 0.7, 1.4, 5, 0);
      }
      // beat 3: it takes the R out of the air and throws it into the bar
      if (t > 1.4 && t < 2.2) { a0.tx = rb.x - U * 0.95; a0.ty = sy - U * 1.25; a0.curlTo = 0; a0.speed = 10; focus = { x: R.x, y: R.y }; }
      if (once("catch", 2.18)) { R.state = "inhand"; a0.curlTo = 0.7; }
      if (once("hideA", 2.1) && V !== 1) A.hidden = true;
      if (t >= 2.18 && t < 2.75) { a0.tx = rb.x + U * 0.35; a0.ty = sy - U * 0.15; a0.speed = 7; a0.curlTo = 0.7; var tg = seq.t1 || label(V === 2 ? ["projects", "research", "people"] : ["research", "projects", "join", "sail"]); if (tg) focus = { x: tg.x, y: tg.y }; }
      if (t >= 2.75 && t < 3.2) { a0.tx = rb.x - U * 1.25; a0.ty = sy - U * 1.6; a0.speed = 26; a0.curlTo = t < 2.88 ? 0.7 : 0; }
      if (once("throw", 2.88)) { seq.t1 = label(V === 2 ? ["projects", "research", "people"] : ["research", "projects", "join", "sail"]); var p = seq.t1 || { x: S.W * 0.6, y: 36 }; fly(R, function () { return V === 2 ? { x: p.x, y: p.y + 30 } : { x: p.x, y: p.y }; }, 0.5, 1.7, 14, U * 0.5); }
      if (R.state === "inhand") { var gr = grip(a0); a0.holding = true; R.x = gr.x; R.y = gr.y; R.rot += (0.3 - R.rot) * Math.min(1, dt * 8); R.sc = 1.7; }
      if (once("hit1", 3.39)) { slowT = 0.35; impact(seq.t1, V !== 2, V === 2 ? "line" : "letters"); R.big = 1; loose(R, rnd(-260, -80), -320, 0.8); if (V === 2) { spill(seq.t1); } }
      if (t > 3.4 && t < 4.0 && seq.t1) focus = { x: seq.t1.x, y: seq.t1.y };
      // aftermath, by variant: A sweeps the desk into PROJECTS and NEWS; B drops the giant A on JOIN; C is the underline break already thrown, then a sweep
      if (V !== 1 && t >= 4.0 && t < 4.9) {
        var k = ease((t - 4.1) / 0.7); divider = 1; a1.tx = rb.x + U * 1.35 - k * U * 3.0; a1.ty = deskY - U * 0.12; a1.speed = 22; a1.curlTo = 1; focus = { x: a1.x, y: deskY };
        if (t > 4.1) { for (var s = 0; s < 2; s++) dust.push({ x: a1.x, y: deskY - 2, vx: rnd(-80, 360), vy: rnd(-300, -40), t: 0, life: rnd(0.2, 0.55), s: rnd(1, 2), g: 900, spark: true });
      ...
      if (V !== 1 && once("hit2", 5.05)) { impact(seq.t2, false, "letters"); loose(seq.shots[0], rnd(-200, 200), -240, 0.8); lamp.va += 3; }
      if (V !== 1 && once("hit3", 5.25)) { impact(seq.t3, false, "letters"); loose(seq.shots[1], rnd(-200, 200), -240, 0.8); }
      if (t > 4.9) a1.curlTo = 0.5;
      if (once("card", 4.6)) card = { x: S.W + 120, y: deskY - 7, vx: -S.W * 1.1, rot: -0.1 };
      if (V === 1 && once("giant", 4.2)) { seq.t4 = label(["join", "news", "sail"]); A.state = "falling"; A.hidden = false; A.gf = 1.25; A.bounced = -2; A.big = 1; A.sc = 2.4; A.x = seq.t4 ? seq.t4.x : S.W * 0.85; A.y = -170; A.vx = 0; A.vy = 420; A.vr = 1.4; A.strike = seq.t4 ? seq.t4.y - 8 : 40; }
      if (V === 1 && A.strike && A.state === "falling" && A.y >= A.strike) { A.strike = 0; impact(seq.t4, true, "letters"); A.vy = -260; A.vx = rnd(-220, -80); A.vr = -5; lamp.va += 4; }
      if (V === 1 && t > 4.2 && t < 5.4) focus = { x: A.x, y: A.y };
      if (t > 5.6) { focus = { x: rb.x - U, y: deskY }; a0.curlTo = a1.curlTo = 0.4; spotTo = 1.15; }
      if (once("settle", 6.4)) { wrecked = true; heatTo = 0.45; root.setAttribute("data-state", "aftermath"); if (panel) { panel.hidden = false; var pb = panel.querySelector("button"); if (pb) pb.focus({ preventScroll: true }); } hintText("Title system offline."); announce("Title system offline. Navigation surface damaged. Press Restore interface to rebuild."); monitor = ["TITLE SYSTEM: OFFLINE", "NAV SURFACE: DAMAGED"]; }
      return focus;
    }
    function spill(lab) {       // variant C: the underline breaks and pieces slide down into the hero
      if (!wreck || !lab) return; wreck.frags.forEach(function (f) { if (f.key === lab.key && f.line) { f.mode = "fall"; f.vx = f.line * rnd(60, 160); f.vy = rnd(-80, 40); f.vr = f.line * 4; } });
      for (var i = 0; i < 6; i++) dust.push({ x: lab.x + rnd(-40, 40), y: lab.y + 14, vx: rnd(-60, 60), vy: rnd(40, 160), t: 0, life: rnd(1, 2.2), s: rnd(2, 4), g: 700, chip: true });
    }
    function restore() {
      if (!wrecked || restoring) return; var sy0 = window.scrollY; restoring = { t: 0 }; wrecked = false; seq = null; divider = 0; heatTo = 0; warmTo = 0; coolTo = 1; spotTo = 1; if (panel) panel.hidden = true; if (restoreBtn) restoreBtn.hidden = true;
      ...
      window.scrollTo(window.scrollX, sy0); window.setTimeout(function () { var first = letters[0] && letters[0].btn; if (first && !first.disabled) first.focus({ preventScroll: true }); }, 2000);
    }
    ...
    function stepLetters(dt) {
      letters.forEach(function (l) {
        if (l.state === "loose") { l.t += dt; l.rot = Math.sin(l.t * 40) * 0.07 * Math.min(1, l.t * 5); if (l.t > 0.32) l.state = "falling"; }
        else if (l.state === "cue") { l.t += dt; var amp = l.t < 1.1 ? Math.min(1, l.t * 3) : Math.max(0, 1 - (l.t - 1.1) * 2); l.x = l.hx + Math.sin(l.t * 46) * 1.4 * amp; l.y = l.hy + Math.sin(l.t * 31) * 0.8 * amp; l.rot = Math.sin(l.t * 38) * 0.02 * amp; if (l.t > 1.7) { lockIn(l, false); } }
          ...
          if (l.y > restY(l) && l.vy > 0) { l.y = restY(l); puff(l.x, 3); if (l.bounced >= 1 || l.vy < 200) { l.state = "rest"; l.vx = l.vy = l.vr = 0; l.gf = 1; l.big = 0; l.slide = spare(l); l.rot = clamp(((l.rot + Math.PI) % 6.2832 + 6.2832) % 6.2832 - Math.PI, -0.55, 0.55); } else { l.bounced++; l.vy *= -0.34; l.vx *= 0.5; l.vr *= 0.4; fx.push({ kind: "tap", x: l.x, y: deskY, t: 0, life: 0.25 }); if (Math.abs(l.vy) > 200) fx.push({ kind: "flash", x: l.x, y: deskY - 4, t: 0, life: 0.12, r: 26 }); } }
    ...
    function lightRgb() { var w = warm * (1 - heat); return [Math.round(255 - cool * 60), Math.round(238 - heat * 125 - w * 50 - cool * 8), Math.round(216 - heat * 140 - w * 120 + cool * 39)].join(","); }
    function drawDesk(ctx) {
      ...
      // the desk front runs under the copy, with the same light falling on it, so the whole hero is one room
      var fg = ctx.createLinearGradient(0, deskY + U * 0.2, 0, S.H); fg.addColorStop(0, "#161a22"); fg.addColorStop(0.5, "#0d1017"); fg.addColorStop(1, "#07090d"); ctx.fillStyle = fg; ctx.fillRect(0, deskY + U * 0.2, S.W, S.H - deskY); ctx.fillStyle = "rgba(0,0,0,.5)"; ctx.fillRect(0, deskY + U * 0.2, S.W, 3);
      var fp = ctx.createRadialGradient(rb.x, deskY + U * 0.2, 10, rb.x, deskY + U * 0.2, S.W * 0.5); fp.addColorStop(0, "rgba(" + rgb + ",.10)"); fp.addColorStop(1, "rgba(" + rgb + ",0)"); ctx.fillStyle = fp; ctx.fillRect(0, deskY + U * 0.2, S.W, S.H - deskY);
      for (var x = S.W * 0.08; x < S.W; x += S.W * 0.21) { ctx.fillStyle = "rgba(255,255,255,.035)"; ctx.fillRect(x, deskY + U * 0.2, 1, S.H); }
      ...
      var hot = monitor.join(" ").indexOf("INTERRUPTION") >= 0 || monitor.join(" ").indexOf("OVERRIDE") >= 0 || monitor.join(" ").indexOf("OFFLINE") >= 0; ctx.font = "600 " + Math.max(8, U * 0.085).toFixed(1) + "px ui-monospace, Consolas, monospace"; ctx.textAlign = "left"; ctx.textBaseline = "alphabetic"; ctx.fillStyle = hot ? "rgba(255,120,110,.95)" : cool > 0.3 ? "rgba(150,200,255,.9)" : "rgba(170,182,196,.8)";
      monitor.forEach(function (line, n) { if (n === 1 && line.indexOf("·") >= 0) { ctx.fillStyle = "rgba(170,182,196,.45)"; ctx.fillText(line, monX - mw / 2 + U * 0.1, my + U * 0.41); ctx.fillStyle = "rgba(170,182,196,.8)"; return; } ctx.fillText(line, monX - mw / 2 + U * 0.1, my + U * (0.24 + (n === 2 ? 1 : n) * 0.17 + (n === 2 ? 0.17 : 0))); }); if (Math.sin(clock * 4) > 0) ctx.fillRect(monX - mw / 2 + U * 0.1, my + U * 0.52, U * 0.06, U * 0.1);
    ...
    function drawTrack(ctx) {
      if (!track) return; var g = ctx.createRadialGradient(track.x, deskY, 4, track.x, deskY, 90 * Z); g.addColorStop(0, "rgba(255,244,225,.22)"); g.addColorStop(1, "rgba(255,244,225,0)"); ctx.fillStyle = g; ctx.beginPath(); ctx.ellipse(track.x, deskY + 2, 90 * Z, 22 * Z, 0, 0, 6.2832); ctx.fill();
    }
    ...
    var api = {
      step: function (dt) {
        ...
        if (!seq && !wrecked && !restoring) {
          var st = level >= 1 ? "warning" : "calm"; if (root.getAttribute("data-state") !== st) { root.setAttribute("data-state", st); if (st === "warning") { hintText("Repeated interruption detected."); announce("Repeated interruption detected."); } }
          warmTo = level >= 1 ? 1 : 0; monitor = level >= 1 ? ["STATE: REPEATED INTERRUPTION", "TASK: restore title"] : ["TASK: restore title", "observe · act · verify · adapt", job ? "STATE: acting" : "STATE: observing"]; if (level >= 1 && Math.random() < dt * 2) flicker = 0.25;
          if (cue) { cue.t += dt; if (cue.t > 2.2) cue = null; }
          else if (!job && cueAt > 0) { cueAt -= dt; if (cueAt <= 0 && letters.every(function (l) { return l.state === "home"; })) { var c = letters[(Math.random() * letters.length) | 0]; c.state = "cue"; c.t = 0; cue = { l: c, t: 0 }; } }
        }
        flicker = Math.max(0, flicker - dt); heat += (heatTo - heat) * Math.min(1, dt * 4); warm += (warmTo - warm) * Math.min(1, dt * 3); cool += (coolTo - cool) * Math.min(1, dt * 2.5); spot += (spotTo - spot) * Math.min(1, dt * 5);
      ...
      draw: function (ctx, S) {
        ...
        ctx.save(); ctx.beginPath(); ctx.rect(0, 0, S.W, deskY + 1); ctx.clip(); drawBody(ctx); drawArms(ctx, true); ctx.restore(); drawDesk(ctx); drawTrack(ctx);
