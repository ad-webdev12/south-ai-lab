...
| Agents and RL | One interaction (`scene-agents.js`): title letters fall and a humanoid robot at a standing desk walks over, picks each one up and sets it back. The robot is a render cut into six pieces (`assets/img/robot/`, joints listed in `RIG` at the top of the script) and posed with two-joint inverse kinematics. Three canvases give the depth: room, then the mounted title, then robot and desk, then loose letters, then hands and debris. Eight knocks within six seconds starts a staged destruction that breaks a visual copy of the navbar (real links stay usable); Restore interface reverses it. The robot image was supplied by Aarav and appears to be a Tesla Optimus press render, so check that the club may use it, or swap in another full-body front-facing robot on a white background and re-cut it. Older versions kept unused: `scene-agents-robot.js` (+ `bot.js`, `memory.js`) and `scene-agents-desk.js`. |
