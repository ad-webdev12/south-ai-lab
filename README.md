...
| Agents and RL | One interaction (`scene-agents.js`): title letters fall and a humanoid robot at a desk walks over, picks each one up and sets it back. Knock eight or more loose within about six seconds and it stages a destruction sequence that breaks a visual copy of the navbar (the real links stay clickable underneath); a Restore interface button puts everything back. Earlier versions are kept unused as `scene-agents-robot.js` (the small adaptive robot, needs `bot.js` and `memory.js`) and `scene-agents-desk.js` (the planning desk). |
