...
| Agents and RL | One interaction (`scene-agents.js`): title letters fall and a humanoid robot at a standing desk walks over, picks each one up and sets it back. The robot is a render cut into six pieces (`assets/img/robot/`, joints listed in `RIG` at the top of the script) and posed with two-joint inverse kinematics. Its hands are drawn, not photographed (`hand()`), so the fingers curl around a letter, the wrist bends, and the palm turns down on the desk. Arms close to the body are foreshortened so the elbows do not wing out, and walking has a step cadence, weight shift and lean (`hip()`, `roll()`). The hero has five states on `data-state` (calm, warning, destruction, aftermath, recovery) that drive the light, the monitor and the hint under the title. Every title letter holds an invisible button, so keyboard users can knock letters loose and a live region announces what happened. The nav wreck is a copy; Home and Join fragments only hang, never leave. The recovery panel is absolutely positioned on purpose: anything that changes the hero's height makes the ResizeObserver rebuild the scene. Three canvases give the depth: room, then the mounted title, then robot and desk, then loose letters, then hands and debris. Eight knocks within six seconds starts a staged destruction that breaks a visual copy of the navbar (real links stay usable); Restore interface reverses it. The robot image was supplied by Aarav and appears to be a Tesla Optimus press render, so check that the club may use it, or swap in another full-body front-facing robot on a white background and re-cut it. Older versions kept unused: `scene-agents-robot.js` (+ `bot.js`, `memory.js`) and `scene-agents-desk.js`. |
